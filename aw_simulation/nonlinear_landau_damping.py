...
if __name__ == "__main__":
    ...
    # make directory
    if not os.path.exists("data/nonlinear_landau_" + str(setup.Nv)):
        os.makedirs("data/nonlinear_landau_" + str(setup.Nv))

    # save results every n steps
    skip = 10

    # save results
    np.save("data/nonlinear_landau_" + str(setup.Nv) + "/sol_u_" + str(setup.Nv) + "_closure_" + str(setup.closure_type) + "_collisions_" + str(setup.col_type) + "_" + str(setup.hyper_rate) + "_nu_" + str(setup.nu), sol_midpoint_u[:, ::skip])
    np.save("data/nonlinear_landau_" + str(setup.Nv) + "/sol_t_" + str(setup.Nv) + "_closure_" + str(setup.closure_type) + "_collisions_" + str(setup.col_type) + "_" + str(setup.hyper_rate) + "_nu_" + str(setup.nu), setup.t_vec[::skip])

    # save parameters
    np.save("data/nonlinear_landau_" + str(setup.Nv) + "/sol_setup_" + str(setup.Nv) + "_closure_" + str(setup.closure_type) + "_collisions_" + str(setup.col_type) + "_" + str(setup.hyper_rate) + "_nu_" + str(setup.nu), setup)
